        ...
        <div className='flex flex-col lg:flex-row lg:gap-6'>
            ...
            <form action="#" onSubmit={handleSubmit} className='flex flex-col py-4 gap-4 md:self-center lg:gap-6 xl:gap-8 xl:w-[400px] xl:items-end'>
                <div className='flex flex-row'>
                    <label htmlFor="name" className='min-w-[64px] md:min-w-[96px] md:text-xl'>İsim</label>
                    <input type="text" id="name" value={name} className='border-2 outline-none px-2' onChange={(e) => setName(e.target.value)} />
                </div>
                <div className='flex flex-row'>
                    <label htmlFor="email" className='min-w-[64px] md:min-w-[96px] md:text-xl'>Email</label>
                    <input type="email" id="email" value={email} className='border-2 outline-none px-2' onChange={(e) => {
                        setEmail(e.target.value);   
                    }}/>
                </div>
                <div className='flex flex-row'>
                    <label htmlFor="message" className='min-w-[64px] md:min-w-[96px] md:text-xl'>Message</label>
                    <textarea name="message" id="message" cols="30" rows="5" className='outline-none border-2 w-[200px] xl:h-[154px] px-2' onChange={(e) => setMessage(e.target.value)}></textarea>
                </div>
